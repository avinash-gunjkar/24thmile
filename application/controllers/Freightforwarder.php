	...
	public function editAnnualContract($annualContractId)
	{
		...
		if ($this->input->post()) {
			...
			foreach ($routes as $route) {
				foreach ($route['charges'] as $charge) {
					$charge['route_id'] = $route['id'];
					$charge['annual_contract_id'] = $annualContractId;
					$charge['ff_company_id'] = $this->seller_session_data['company_id'];
					if (empty($charge['id'])) {
						//insert
						if($this->annual_contract_route_rfc_charges_model->insert($charge) && !$update){
							$update = true;
						}
						
					} else {
						//update
						if($this->annual_contract_route_rfc_charges_model->update($charge['id'],['charges'=>$charge['charges']]) && !$update){
							$update = true;
						}
			...
			redirect(base_url('ff-edit-annual-contract/'.$annualContractId));
		}
