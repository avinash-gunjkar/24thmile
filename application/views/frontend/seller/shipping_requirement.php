                                    ...
                                    <div class="form-group">
                                        ...
                                        <div id="multiPackaging" class="multi-packaging hideFCL mb-3" style="<?= $requestDetails->shipment_id == '1' || empty($requestDetails->shipment_id) ? 'display:none;' : ''; ?>">
                                            ...
                                            <?php if (!empty($requestDetails->package)) { ?>
                                                ...
                                                <?php } ?>
                                            <?php } else { ?>
                                                <?php $this->load->view('frontend/ajax/ajaxAddPackage', ['packageCounter' => 1, 'item_details' => null, 'packingList' => $packingList]); ?>
                                            <?php } ?>
...
<div class="modal fade" id="selectBranchAddress" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Select Branch Address</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
                    ...
                    <tbody>
                        <?php foreach ($compnayBranch as $key => $branch) { ?>
                            <tr style="cursor: pointer">
                                ...
                                <td><input type="button" class="btn btn-sm btn-primary btn-select" onclick="selectAddress(<?= $key ?>)" value="Select" /></td>
                            </tr>
                        <?php } ?>
...
<div class="modal fade" id="selectConsigneeAddress" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Select Consignee Address</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
                    ...
                    <tbody>
                        <?php foreach ($consigneeAddressList as $key => $branch) { ?>
                            <tr style="cursor: pointer">
                                ...
                                <td><?= $branch->fax ?></td>
                                <td><?= $branch->contact_person ?></td>
                                <td><input type="button" class="btn btn-sm btn-primary btn-select" onclick="selectConsigneeAddress(<?= $key ?>)" value="Select" /></td>
                            </tr>
                        <?php } ?>
                        <?php if (empty($compnayBranch)) { ?>
                            <tr>
...
<script type="text/javascript">
    ...
    $(document).on('blur', '.length,.width,.height', function() {
        ...
        var volume_meter = (volume_cm / 1000000).toFixed(1);
    ...
    function selectAddress(index) {
        ...
        $('#consignor_company_name').val(branchlist[index].branch_name);
        $('#consignor_address_line_1').val(branchlist[index].address_line_1);
        $('#consignor_address_line_2').val(branchlist[index].address_line_2);
        $('#consignor_city_name').val(branchlist[index].city_name);
        $('#consignor_city_id').val(branchlist[index].city_id);
        $('#consignor_state_id').val(branchlist[index].state_id);
        $('#consignor_country_id').val(branchlist[index].country_id);
        $('#consignor_name').val(branchlist[index].contact_person);
        $('#consignor_pincode').val(branchlist[index].pincode);
        $('#consignor_phone').val(branchlist[index].phone);
        $('#consignor_country_code').val(branchlist[index].contact_country_code);
        $('#consignor_email').val(branchlist[index].email);
        $('#selectBranchAddress').modal('hide');
    }
    function selectConsigneeAddress(index) {

        $('#consignee_company_name').val(consignorAddresslist[index].branch_name);
        $('#consignee_address_line_1').val(consignorAddresslist[index].address_line_1);
        $('#consignee_address_line_2').val(consignorAddresslist[index].address_line_2);
        $('#consignee_city_name').val(consignorAddresslist[index].city_name);
        $('#consignee_city_id').val(consignorAddresslist[index].city_id);
        $('#consignee_state_id').val(consignorAddresslist[index].state_id);
        $('#consignee_country_id').val(consignorAddresslist[index].country_id);
        $('#consignee_name').val(consignorAddresslist[index].contact_person);
        $('#consignee_pincode').val(consignorAddresslist[index].pincode);
        $('#consignee_phone').val(consignorAddresslist[index].phone);
        $('#consignee_country_code').val(consignorAddresslist[index].contact_country_code);
        $('#consignee_email').val(consignorAddresslist[index].email);
    ...
    function getShipmentSummary() {
        ...
        $('.shipment-summary .total-volume span').text(totalVolume.toFixed(1));
