...
class Shipment_documents extends CI_Model{
	...
	public function getRecord($request_id, $document_type, $company_id){
		$this->db->select('*');
    ...
    //return list of documents based on request_id, document_type, company_id
  	public function getList($request_id, $company_id,$filter=[]){
		$this->db->Select("tbl_shipment_document_master.document,tbl_shipment_document_master.name,$this->TBL.id, $this->TBL.invoice_number,$this->TBL.invoice_date,$this->TBL.invoice_amount, $this->TBL.status");
		// $this->db->where('request_id',$request_id);
		// $this->db->where('company_id',$company_id);
		...
		$this->db->join('tbl_shipment_document_master',"$this->TBL.document_type = tbl_shipment_document_master.document AND request_id = $request_id AND company_id = $company_id ","RIGHT");
		$query = $this->db->get($this->TBL);
